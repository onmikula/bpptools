...
read_bpp_output <- function(mcmc, starting=FALSE, thinning=1, tree=NULL) {
	...
	if (A00 == FALSE) {
		...
		trees <- lapply(trees, reorder, order="postorder")
...
combine_bpp_runs <- function(runs) {
	...
	params <- Reduce(rbind, lapply(runs, "[[", "params"))
	nspecies <- Reduce(c, lapply(runs, "[[", "nspecies"))
	if (length(nspecies) < nrow(params)) {
		nspecies <- nspecies[1]
	}
	result <- list(trees=trees, params=params, nspecies=nspecies)
	class(result) <- "bpp"
...
gdi <- function(params, tree) {
	...
	pgdi <- pgdi[,-(ntip+1)]
	mgdi <- round(apply(pgdi, 2, mean), 6)
...
estimate_params <- function(seq, imap=NULL, model="raw") {
	...
	for (i in seq(length(otus) - 1)) {
		for (j in (i+1):length(otus)) {
			tau[i,j] <- mean(dst[otus[[i]],otus[[j]]])
		}
	}
...
#	clade: vector of node labels; if not tree$clade.label (default) it requires node numbers as its names 
#	species: vector of edge labels; if notdelim$PP (default) it requires node numbers as its names 
...
plot_msc_tree <- function(tree, gap=0.1, col="grey", border="black", lwd=3, cex=1.5, show.tip.label=TRUE, tip.label=NULL, tip.cex=cex, tip.font=3, timescale=TRUE, axis.lwd=lwd/2, labels="theta", digits=4, lab.col="white", lab.border="black", lab.lwd=lwd/2, lab.cex=1, lab.font=1, xlab="Tau", ylab="Theta", xlim, ylim, mai, direction, device, file, width=7, height=7) {
	...
	par(mai=mai)
	plot(0, xlim=xlim, ylim=ylim, asp=1, bty="n", xaxt="n", yaxt="n", xlab="", ylab="")
	for (i in nodes) {
		ii <- which(tree$edge[,1] == i)
	...
	if (isTRUE(timescale)) {
		tau <- rev(axTicks(side=1, usr=xlim))
		d <- max(tau) - xlim[2]
		at <- rev(tau) - d
		if (d < 0) {
			decim <- max(nchar(gsub("^0*\\.", "", tau)))
			tau <- c(round(max(bc$xx), decim), tau)
			at <- c(max(at) - tau[1], at)
		}
