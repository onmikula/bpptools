...
#	indnames: whether to use individual rather than sequence names to label sequences (useful for functions like 'estimate_otus' or 'estimate_priors')
# Value:
...
read_bpp_imap <- function(imapfile, col.names=c("Individual", "Species")) {
	read.table(imapfile, header=FALSE, col.names=col.names)
}



# Function:
#	read_bpp_priors: reads BPP imap file
# Arguments:
#	ctlfile: the name of control file
#	which: prior identification ("theta" & "tau" by default)
# Value:
#	data frame with columns 'alpha', 'beta', 'mean' & 'distribution' (the latter giving the type of distribution) and possibly 'ncat' (for the discretized gamma)

read_bpp_priors <- function(ctlfile, which=c("tau","theta")) {
	spaces <- "[[:punct:][:space:]]+"
...
read_bpp_output <- function(mcmc, thinning=1, tree=NULL) {
...
parse_bpp_ctlfile <- function(con, spdelim, sptree, seqfile, imapfile, outfile, mcmcfile, species, tree, nloci, thetaprior, tauprior, thetadistr ="invgamma", taudistr ="invgamma", theta=TRUE, phiprior=c(1,1), spdelimalgorithm=c(0, 2), speciesmodelprior=1, phase=FALSE, model="JC69", gamma=FALSE, alphaprior=c(1,1,4), locusrate=0, clock=1, heredity=0, cleandata, finetune=1, print=c(1,0,0,0,0), burnin=20000, sampfreq=10, nsample=20000, seed=-1, usedata=TRUE, threads=NULL) {
	...
	thetaprior <- thetaprior[1:2]
	tauprior <- tauprior[1:2]
	if (isTRUE(theta)) {
...
gdi <- function(params, tree, hpd=0.95) {
	...
	mgdi[is.nan(mgdi)] <- NA
	cint <- round(t(apply(pgdi, 2, HPD, p=hpd)), 6)
	
	return(list(means=mgdi, distributions=pgdi, hpd=cint))
}
...
estimate_otus <- function(seq, model="raw") {
	...
	if (is.list(seq)) {
		...
		concatenated <- matrix("N", length(rows), max(bp), dimnames=list(rows, NULL))
...
# Function:
#	estimate_priors: estimate putative means of theta and tau prior distributions
# Arguments:
#	seq: list of alignments with row names indicating individuals or a single alignment of such sequences (possibly concatenated multi-locus data)
#	imap: matrix or data frame (1st column individuals, 2nd column species) or a name of imap file
#	d_tau, d_theta: either 'invgamma' (default) 'gamma'
#	d_theta: matrix or data frame (1st column individuals, 2nd column species) or a name of imap file
#	a_tau, a_theta: alpha parameters of the priors (default is 3 for the inverse gamma, but not specified for gamma)
#	model: nucleotide substitution model indicated as in 'ape::dist.dna'
# Value:
#	data frame with columns 'alpha', 'beta', 'mean' & 'distribution' (the latter giving the type of distribution)
# Depends on: ape

estimate_priors <- function(seq, imap=NULL, d_tau="invgamma", d_theta="invgamma", a_tau, a_theta, model="raw") {
	if (is.list(seq)) {
		bp <- cumsum(sapply(seq, ncol))
		part <- cbind(c(1, bp[-length(bp)] + 1), unname(bp))
		rows <- sort(unique(unlist(lapply(seq, rownames))))
	...
	a_tau <- ifelse(d_tau == "invgamma" & missing(a_tau), 3, NA)
	b_tau <- ifelse(d_tau == "invgamma", (a_tau - 1) * m_tau, a_tau / m_tau)
	a_theta <- ifelse(d_theta == "invgamma" & missing(a_theta), 3, NA)
...
avedist <- function(seq, imap, model="K80", diag=TRUE) {
	...
	seq <- seq[intersect(rownames(seq), imap[,1]),]
...
plot_msc_tree <- function(tree, asp=1, gap=0.1, col="grey", border="black", lwd=3, cex=1.5, show.tip.label=TRUE, tip.label=NULL, tip.cex=cex, tip.font=3, timescale=TRUE, axis.lwd=lwd/2, labels="theta", digits=4, lab.col="white", lab.border="black", lab.lwd=lwd/2, lab.cex=1, lab.font=1, xlab="Tau", ylab="Theta", xlim, ylim, mai, direction, device, file, width=7, height=7) {
	...
	gapsize <- gap
	gap <- gapsize * max(params[,"theta"], na.rm=TRUE)
	...
	if (gap > gapsize * max(params[,"theta"], na.rm=TRUE)) {
		bc <- branch_coords(tree, params, gap)
	}
...
plot_pdensity <- function(prior, posterior, distribution="invgamma", qmax=0.99, points=FALSE, q=c(0.025, 0.5, 0.975), xlim=NULL, xlab="", ylab="density", col=c(1,8), lty=c(3,1), lwd=2, cex=1.5, legend=TRUE, expr) {
...
plot_gdi <- function(gdis, which=1, hpd=0.95, col=1, bg=8, lwd=4, cex=2, points=TRUE, xlab="gdi", ylab="density", mai, cex.lab=1.5, cex.axis=1.25, expr) {
	r <- range(gdis$distributions[,which])
	d <- stats::density(gdis$distributions[,which], from=r[1], to=r[2])
