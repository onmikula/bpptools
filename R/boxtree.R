...
# shifts of species branches
	for (anc in which(tree$edge[,2] %in% nodes)) {
		...
		if (shifts > 0) {
			...
			yy[branches1,] <- yy[branches1,] - shifts
			branches2 <- unique(c(off[2], unlist(edg[off2])))
			yy[branches2,] <- yy[branches2,] + shifts
		}
	}
...
# overlaps of both sister & non-sister branches & shifts of the subclades they belong to
	...
	for (sec in sectimes) {
		...
		gaps <- -1 * sapply(branchpairs, function(ii) diff(yy[ii,][order(rowMeans(yy[ii,])),][2:3]))
		while (any(gaps < 0)) {
			minpair <- branchpairs[[which.min(gaps)]]
			mrca <- ape::getMRCA(tree, tree$edge[minpair,2])
			basal <- edg[tree$edge[tree$edge[,1] == mrca,2]]
			basal <- basal[order(rowMeans(yy[tree$edge[,1] == mrca,]))]
			shifts <- - 0.5 * min(gaps) + gap * max(params[tree$edge[minpair,2],"theta"])
...
# unification of terminal gaps
	branchpairs <- match(seq(ntip),tree$edge[,2])
	branchpairs <- Map(c, branchpairs[-ntip], branchpairs[-1])
	branchpairs <- branchpairs[sapply(branchpairs, function(ii) tree$edge[ii[1],1] != tree$edge[ii[2],1])]
	gaps <- -1 * sapply(branchpairs, function(ii) diff(yy[ii,][order(rowMeans(yy[ii,])),][2:3]))
	tgap <- round(max(gaps), 8)
	while (any(gaps < tgap)) {
		minpair <- branchpairs[[which.min(gaps)]]
		mrca <- ape::getMRCA(tree, tree$edge[minpair,2])
		basal <- edg[tree$edge[tree$edge[,1] == mrca,2]]
		basal <- basal[order(rowMeans(yy[tree$edge[,1] == mrca,]))]
		shifts <- 0.5 * (tgap - min(gaps))
		yy[basal[[1]],] <- yy[basal[[1]],] - shifts
		yy[basal[[2]],] <- yy[basal[[2]],] + shifts
		gaps <- round(-1 * sapply(branchpairs, function(ii) diff(yy[ii,][order(rowMeans(yy[ii,])),][2:3])), 8)
	}
...
# graphical device
	...
	if (!is.null(device) & !isTRUE(is.na(device))) {
		if (!device %in% c("quartz", "x11")) {
			isfile <- TRUE
			if (missing(file)) {
				file <- paste0("msc_tree.", sub("^.*_", "", device))
			}
			devname <- file
...
# labels
	if (is.null(labels)) {
		labels <- NA
	} else if (labels %in% colnames(params)) {
		...
		labels <- NA
		warning("the specified 'labels' are not among the estimated parameters")
	}
