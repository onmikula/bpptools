	...
	if (im) {
		...
		if (!sm) {
			if (al) {
				...
				imap <- data.frame(Individual=unique(indnames), Species=unique(indnames))
				seqmap <- data.frame(Sequence=rownames(seq), Individual=indnames)
			} else {
				imap <- data.frame(Individual=rownames(seq), Species=rownames(seq))
				seqmap <- data.frame(Sequence=rownames(seq), Individual=rownames(seq))
			}
		} else {
			seqmap <- setNames(as.data.frame(seqmap[,1:2]), c("Sequence", "Individual"))
			imap <- unique(data.frame(Individual=seqmap$Individual, Species=seqmap$Individual))
		}
	}

	rows <- lapply(seqmap$Sequence, grep, x=rownames(seq))
