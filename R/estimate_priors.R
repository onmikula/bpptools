...
#' 
#' @param dat A data frame or a matrix containing posterior samples of 'tau' (1st column) or 'theta' (2nd column)
...
#' @param p Numeric, the quantile of the highest posterior density used for estimation from posterior samples.
#' @param round Logical, whether to round beta parameters of gamma priors to integer values.
...
estimate_priors <- function(dat, imap=NULL, d_tau, d_theta, a_tau, a_theta, seqmap=NULL, allele=NULL, model="JC69", p=0.95, round=TRUE) {

	priors_from_samples <- function(dat, d_tau, d_theta, a_tau, a_theta, p, round) {
		dat <- as.matrix(dat)
		taucol <- grep("tau", colnames(dat), ignore.case=TRUE)
		thetacol <- grep("theta", colnames(dat), ignore.case=TRUE)
		if (length(taucol) > 0 & length(thetacol) > 0) {
			dat <- dat[,c(taucol, thetacol)]
		} else {
			dat <- dat[,1:2]
		}
		for (i in 1:2) {
			int <- HPD(dat[,i], p=p)
			out <- dat[,i] < int[1] | dat[,i] > int[2]
			dat[out,i] <- NA
		}
		m_tau <- mean(dat[,1], na.rm=TRUE)
		m_theta <- mean(dat[,2], na.rm=TRUE)
	...
	priors_from_sequences <- function(dat, imap, d_tau, d_theta, a_tau, a_theta, seqmap, allele, model, round) {
		if (is.list(dat)) {
			dat <- concatenate(dat, class="DNAbin")
		} else if (!inherits(dat, "DNAbin")) {
			dat <- ape::as.DNAbin(dat)
		}
		im <- !is.null(imap)
		sm <- !is.null(seqmap)
		al <- !is.null(allele)
		if (im) {
			...
			if (!sm) {
				if (al) {
					...
					seqnames <- lapply(seqnames, grep, x=rownames(dat), value=TRUE)
				...
				seqnames <- lapply(seqmap$Sequence, grep, x=rownames(dat), value=TRUE)
				seqmap <- unique(data.frame(Sequence=unlist(seqnames), Individual=rep(seqmap$Individual, sapply(seqnames, length))))
			}
		} else {
			if (!sm) {
				if (al) {
					indnames <- sub(allele, "", rownames(dat))
					imap <- data.frame(Individual=unique(indnames), Species=NA)
					seqmap <- data.frame(Sequence=rownames(dat), Individual=indnames)
				} else {
					imap <- data.frame(Individual=rownames(dat), Species=NA)
					seqmap <- data.frame(Sequence=rownames(dat), Individual=rownames(dat))
				}
			} else {
				seqmap <- setNames(as.data.frame(seqmap[,1:2]), c("Sequence", "Individual"))
		...
		if (!im) {
			otus <- estimate_otus(dat, seqmap=seqmap, allele=allele, model=model)
			imap$Species <- otus$Species[match(imap$Individual, otus$Individual)]
		}
		dst <- ape::dist.dna(dat[seqmap$Sequence,], pairwise.deletion=TRUE, model=model, as.matrix=TRUE)
		dst[dst == 0] <- 1e-8
		...
		m_theta <- mean(dst[outer(otus, otus, "==") & upper.tri(dst)], na.rm=TRUE)
		means <- c(m_tau, m_theta)
		b_tau <- ifelse(d_tau == "invgamma", (a_tau - 1) * m_tau, a_tau / m_tau)
		b_theta <- ifelse(d_theta == "invgamma", (a_theta - 1) * m_theta, a_theta / m_theta)
		if (isTRUE(round)) {
			round <- c(gamma=TRUE, invgamma=FALSE)[c(d_tau, d_theta)]
		} else {
			round <- c(FALSE, FALSE)
		}
		b_tau <- ifelse(isTRUE(round[1]), round(b_tau), b_tau)
		b_theta <- ifelse(isTRUE(round[2]), round(b_theta), b_theta)	
		priors <- data.frame(alpha=round(c(a_tau, a_theta), 5), beta=round(c(b_tau, b_theta), 5), mean=round(c(m_tau, m_theta), 5), distribution=c(d_tau, d_theta), row.names=c("tau", "theta"))
		return(priors)
	}

	samples <- is.data.frame(dat) | (is.matrix(dat) & is.numeric(dat) & !inherits(dat, "DNAbin"))
	if (isTRUE(samples)) {
		priors <- priors_from_samples(dat, d_tau, d_theta, a_tau, a_theta, p=p, round=round)
	} else {
		priors <- priors_from_sequences(dat, imap=imap, d_tau, d_theta, a_tau, a_theta, seqmap=seqmap, allele=allele, model=model, round=round)
	}
