...
#'
#' `speciesPP` takes an output of `speciesDA` and converts it into a list with components
#'   `"delims"` (data frame with delimitations and their posterior probabiities, PPs),
#'   `"species"` (data frame with species and PPs) and `"tips"` (candidate species ordered
#'   as in column names of `speciesDA` output). 
#'
#' `speciesCON`, `speciesMAP`, `speciesMC`, functions choosing the majority consensus, the maximum a posteriori,
#'   or the maximum credibility species delimtation, respectively. They all return a data frame with species
...
#' @param type The type of delimitation, either `"CON"` (majority consensus, default),
...
find_spdelim <- function(trees, file, type=c("CON","MAP","MC"), sep="-", collapse=0) {
...
speciesPP <- function(psample, sep="-") {
	...
	delims <- data.frame(delim=names(delims), pp=as.numeric(delims), stringsAsFactors=FALSE)
...
speciesCON <- function(pp, sep="-") {
	...
	CON <- pp$species[1:i,]
	delim <- strsplit(CON[,1], split=sep)
	...
	attr(CON, "pp") <- pp$delims[match(delim, pp$delims[,1]),2]
	return(CON)
}
