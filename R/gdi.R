...
#' 
#' @param params An object of class `bpp` or a data frame with multispecies coalescent parameters.
#' @param tree An object of class `phylo` or a tree in Newick format obtained by `read_bpp_tree`
...
#	tree <- ape::reorder.phylo(ape::read.tree(text=tree), order="postorder")
	...
	mgdi[is.nan(mgdi)] <- NA
	if (!is.na(interval)) {
		cint <- round(t(apply(pgdi, 2, match.fun(interval), p=p)), 5)
	} else {
		cint <- NULL
	}
