...
plot_traces <- function(mcmc, param, device, pdf, width=7, height, palette, mai, title=TRUE, lwd=1, ...) {
...
plot_samples <- function(mcmc, param, interval="HPD", p=0.95, device, pdf, width, height=7, palette, mai, title=TRUE, lwd=1, bg="white", pch=21, cex=1.5, ...) {
...
plot_runs <- function(mcmc, param, type, interval="HPD", p=0.95, device, pdf, width, height, palette, mai, title=TRUE, lwd=1, bg="white", pch=21, cex=1.5, ...) {
	...
	if (length(type) == 1) {
		...
		if (type == "traces") {
			...
			plot_traces(mcmc=mcmc, param=param, device=device, pdf=pdf, width=width, height=height, palette=palette, mai=mai, title=title, lwd=lwd, ...)
		}
		if (type == "samples") {
			if (missing(width)) {
				width <- nrun * 1 + 0.5
			...
			plot_samples(mcmc=mcmc, param=param, interval=interval, p=p, device=device, pdf=pdf, width=width, height=height, palette=palette, mai=mai, title=title, lwd=lwd, bg=bg, pch=pch, cex=cex, ...)
		}
	} else {
		if (is.character(mcmc)) {
			mcmc <- lapply(mcmc, read_bpp_mcmc) 
