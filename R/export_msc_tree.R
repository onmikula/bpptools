...
export_msc_tree <- function(target, trees, params, file, treesfile, fun="mean", interval="HPD", p=0.95, gdi=FALSE, figtree=FALSE, constraint) {
	...
	fun <- match.fun(fun)
	int <- match.fun(toupper(interval))
	...
	params <- params[,grepl("^theta.{0,1}[[:digit:]]+|^tau.{0,1}[[:digit:]]+", colnames(params)),drop=FALSE]
	thetanum <- gsub("theta\\.", "", colnames(params)[seq(ntip)])
	thetanum <- as.numeric(gsub("\\..*$", "", thetanum))
	thetanum <- intersect(thetanum, seq(ntip))
