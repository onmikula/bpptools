...
plot_bpp_tree(tree, delim)

# plot of posterior probability density of tau0 and mean theta
posterior <- read.delim(logfile)
plot_pdensity(posterior=posterior[,c("tau.0","mtheta")], xlab="Tau / Theta", col=c(1,8), lty=1, lwd=2, cex=1.5, legend=c("Tau","Theta"))

# plot of prior vs. posterior probability density of tau0 and mean theta, respectively.
priors <- read_bpp_prior(ctlfile)
plot_pdensity(prior=priors["tau",1:2], posterior=posterior[,"tau.0"], xlab="Tau", col=1, lty=c(3,1), lwd=2, cex=1.5, points=FALSE, legend=TRUE)
plot_pdensity(prior=priors["theta",1:2], posterior=posterior[,"mtheta"], xlab="Theta", col=1, lty=c(3,1), lwd=2, cex=1.5, points=FALSE, legend=TRUE)
...
tiplab <- c(H="harennensis", A="triton_A", B="triton_B", "C-D"="triton_CD", E="triton_E")
